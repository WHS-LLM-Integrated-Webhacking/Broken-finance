...
def support():
    ...
    if request.method == 'POST':
        ...
        response = requests.post(API + '/api/support/chatbot', json=data, headers=headers)
        answer = response.json()
        if response.status_code == 201: return jsonify(answer)
        else: return jsonify({'response': None})
